				...
				<form action="alter_book" method="post" enctype="multipart/form-data">
					...
					<c:choose>
						<c:when test="${!empty book.photo }">
							<p>当前图片：<img id="preview" src="${pageContext.request.contextPath}/static/file/${book.photo}" width="100px" height="150px" align="top"/></p>
						</c:when>
						<c:otherwise>
							<p>当前图片：<img id="preview" src="" style="display: none; width:100px; height:150px;align:top;"/></p>
						</c:otherwise>
					</c:choose>
					<p>图书图片：<input type="file" name="photo" onchange="viewImage(this)" value="${book.photo }"  width="100px" height="150px" align="top"/></p>   				 				
